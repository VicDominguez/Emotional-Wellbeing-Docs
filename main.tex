% Para impresión, poner twoside. Para PDF, poner oneside.
\documentclass[a4paper,oneside,12pt,titlepage]{report}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Importación de librerías                        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\usepackage{graphicx}
\usepackage[utf8]{inputenc}
% Para tener los elementos de texto en español. El flag noshorthands arregla el bug de Argument of \language@active@arg" has an extra }.
% http://algunostutoriales.blogspot.com/2014/03/arreglar-un-extrano-error-de-latex.html
\usepackage[english,spanish,es-noshorthands]{babel}
% Hipervinculos para todo lo útil
\usepackage{hyperref}
% Evitamos que rompa las palabras
\usepackage[htt]{hyphenat}
% Soporte para anexos
\usepackage{appendix}
% Paquetes para figuras
\usepackage{caption}
\usepackage{subcaption}
\usepackage{lscape}
% Insertar codigo en memoria
\usepackage{listings}
\usepackage[dvipsnames,x11names,table]{xcolor} % Importa los colores para el codigo y la timeline

% Posicionamiento
\usepackage{float}
% Tablas
\usepackage{xltabular}
% Tablas de más de una página
\usepackage{longtable} 
% Metadatos
\usepackage{metainfo}
% Condicionales (para tener distintas versiones del documento)
\usepackage{etoolbox}
% Fracciones
\usepackage{xfrac}
% Márgenes reducidos
\usepackage[pdftex,scale={.8,.8}]{geometry}


% Bibiografia
\usepackage[style=ieee]{biblatex}
\bibliography{contenido/tfm.bib}

\usepackage{csquotes}
\usepackage{titlesec}

% Importamos ajustes y metadatos
\input{contenido/varios/ajustes.tex}
\input{contenido/varios/metadata.tex}
\input{contenido/varios/tl.tex}


% Comenzamos el documento
\begin{document}
    
    % Hacemos la portada
    \input{contenido/varios/portada.tex}

    % Para comenzar la numeracion de paginas en numeros romanos
    \pagenumbering{Roman} 

    % Agradecimientos
    \input{contenido/inicio/agradecimientos.tex}

    % Hacemos los índices
    \tableofcontents
    \newpage
    \listoftables
    \newpage
    \listoffigures
    \newpage
    \lstlistoflistings
    \newpage

    % Después de los índices ya podemos poner el espacio entre párrafos igual al interlineado
    \setlength{\parskip}{\baselineskip} 
    \pagestyle{long}

    % contenido adicionales antes del inicio
    \chapter*{Glosario}
    \input{contenido/inicio/glosario.tex}
    \newpage
    \chapter*{Resumen}
    \input{contenido/inicio/resumen.tex}
    \chapter*{Abstract}
    \input{contenido/inicio/abstract.tex}
    
    % Ya tenemos las páginas de puro contenido, pasamos al formato normal
    
    \pagenumbering{arabic}

    %Importamos los capítulos
    \chapter{Introducción}
    \input{contenido/introduccion.tex}
    \newpage

    \chapter{Contexto}
    \input{contenido/contexto.tex}
    \newpage

    \chapter{Marco teórico}
    \input{contenido/teorico.tex}
    \newpage

    \chapter{Especificación de requisitos}
    \input{contenido/requisitos.tex}
    \newpage

    \chapter{Diseño del sistema}
    \input{contenido/diseno.tex}
    \newpage

    \chapter{Implementación del sistema}
    \input{contenido/implementacion.tex}
    \newpage

    \chapter{Pruebas del sistema}
    \input{contenido/pruebas.tex}
    \newpage

    \chapter{Resultados}
    \input{contenido/resultados.tex}
    \newpage

    \chapter{Gestión del proyecto}
    \input{contenido/gestion.tex}
    \newpage

    \chapter{Conclusiones}
    \input{contenido/conclusiones.tex}
    \newpage

    \chapter{Lineas futuras}
    \input{contenido/futuras.tex}
    \newpage

    % Bibilografía
    \printbibliography[heading=bibintoc]

    % Anexos
    \appendix
    \clearpage

    % Anexo de código
    \chapter{Código fuente del proyecto} 
    \input{contenido/anexos/codigo.tex}
    \newpage

\end{document}